...
DFT_EXPORT void DFTUndiff_EXECUTE(void *p2, void *s2, int32_t dir) {
...
DFT_EXPORT void DFTUndiff_DESTROYPLAN(void *p2) {
  DFTUndiff *plan = (DFTUndiff *)p2;
  if (plan->magic != MAGIC_DFT) abort();

  SIMDBase_alignedFree(*(plan->ptTable));
  free(plan->ptTable);
  free(plan->cobraT);
  free(plan->cobraR);
  //free(plan->t);
  if (plan->rtTable != NULL) {
    SIMDBase_alignedFree(plan->rtTable[0]);
    SIMDBase_alignedFree(plan->rtTable[1]);
    free(plan->rtTable);
  }
...
DFT_EXPORT DFTUndiff *DFTUndiff_MAKEPLANSUB(uint64_t n, int32_t radix2thres, int32_t useCobra, uint64_t flags) {
  int32_t i, j, k;
  ...
  DFTUndiff *d = (DFTUndiff *)calloc(1, sizeof(DFTUndiff));
  ...
  SIMDBase_REAL *trigTable = (SIMDBase_REAL *)SIMDBase_alignedMalloc(sizeof(SIMDBase_REAL)*n * 2);
  d->ptTable = (SIMDBase_REAL **)malloc(sizeof(SIMDBase_REAL *) * (d->log2len + 1));
  ...
  if (cobraQ >= 4 && d->log2len >= 2*cobraQ) {
	...
	cobraT = (SIMDBase_VECT*)SIMDBase_alignedMalloc(sizeof(SIMDBase_VECT) * 2 * (1 << (cobraQ * 2)));
  ...
  if ((d->flags & DFT_FLAG_REAL) != 0 || (d->flags & DFT_FLAG_ALT_REAL) != 0) {
	...
	d->rtTable = (SIMDBase_REAL**)malloc(sizeof(SIMDBase_REAL *) * 2);
	d->rtTable[0] = (SIMDBase_REAL*)SIMDBase_alignedMalloc(sizeof(SIMDBase_REAL)*m / 2);
	d->rtTable[1] = (SIMDBase_REAL*)SIMDBase_alignedMalloc(sizeof(SIMDBase_REAL)*m / 2);
  ...
  return (DFTUndiff*)((void *)d);
}

DFT_EXPORT void *DFTUndiff_MAKEPLAN(uint64_t n, uint64_t flags) {
  ...
  SIMDBase_REAL *s1 = (SIMDBase_REAL*)SIMDBase_alignedMalloc(sizeof(SIMDBase_VECT)*n * 2);
  ...
  if (flags & DFT_FLAG_VERBOSE) {
    ...
    double nf = 5 * n * log((SIMDBase_REAL)n) / log((SIMDBase_REAL)2) / (t * 1000000);
